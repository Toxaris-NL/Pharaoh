...
namespace Pharaoh
{
    ...
    public class SpriteGameObject : GameObject
    {
        ...
        public int SheetIndex
        {
            get
            {
                return _sprite.SheetIndex;
            }

            set
            {
                _sprite.SheetIndex = value;
            }
        }
        ...
        public SpriteGameObject(string spriteName, float depth, int sheetIndex = 0)
        {
            ...
            if (spriteName != null)
            {
                _sprite = new SpriteSheet(spriteName, depth, sheetIndex);
            }
            Origin = Vector2.Zero;
        }
        ...
        public bool HasPixelPreciseCollision(Rectangle other)
        {
            ...
            for (int x = 0; x < b.Width; x++)
            {
                for (int y = 0; y < b.Height; y++)
                {
                    ...
                    if (!_sprite.IsPixelTransparent(thisX, thisY))
                        return true;
                }
            }
        ...
        public bool HasPixelPreciseCollision(SpriteGameObject other)
        {
            ...
            for (int x = 0; x < b.Width; x++)
            {
                for (int y = 0; y < b.Height; y++)
                {
                    ...
                    if (!_sprite.IsPixelTransparent(thisX, thisY) && !other._sprite.IsPixelTransparent(otherX, otherY))
                        return true;
                }
            }
